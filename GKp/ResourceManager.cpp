...
void ResourceManager::loadShader(string vShaderFile, string fShaderFile, string gShaderFile,string name)
{
	try {
		...
		string vsCodeStr = readFile(vShaderFile);
		string fsCodeStr = readFile(fShaderFile);
		string gsCodeStr = readFile(gShaderFile);
...
{
	char path[256];
	char *pre = "assets/objects/";
	std::vector < glm::vec3* > vertices;
	std::vector < glm::vec3* > normals;
	std::vector < glm::vec2* > textureCords;
	std::vector < string* > faces;
	sprintf(path, "%s%s", pre, file);
	//string objinfo = readFile(path);
...
string ResourceManager::readFile(string filePath)
{
